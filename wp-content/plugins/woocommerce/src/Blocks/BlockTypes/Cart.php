	...
	public function register_patterns() {
		...
		register_block_pattern(
			...
			array(
				...
				'inserter' => true,
				'content'  => '
					<h2 class="wp-block-heading has-text-align-center with-empty-cart-icon wc-block-cart__empty-cart__title">' . esc_html__( 'ТЕСТ 2' ) . '</h2>
					<!-- wp:paragraph {"align":"center"} --><p class="has-text-align-center"><a href="' . esc_attr( esc_url( $shop_permalink ) ) . '">' . esc_html__( 'НЕповернутись в магазин' ) . '</a></p><!-- /wp:paragraph -->
				',
			)
		);
